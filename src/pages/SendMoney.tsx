...
type Step = 'recipient' | 'amount' | 'purpose' | 'type' | 'pin' | 'confirm' | 'done';
...
export function SendMoney() {
  ...
  const [transferType, setTransferType]     = useState<'standard' | 'protected'>('standard');
  ...
  const tapMic = async () => {
    ...
    try {
      if (step === 'recipient') {
        ...
        const resp = await prompt('What is the purpose of this transfer?');
        if (resp) setPurpose(resp);
      }
    } catch { /* no mic */ }
    setMicLoading(false);
  };
  ...
  const prevStep = (): Step => {
    const order: Step[] = ['recipient','amount','purpose','type','pin','confirm','done'];
    ...
    if (step === 'recipient') {
      ...
      if (!recipientPhone) {
        const found = allUsers.find(
          u => u.accountNumber === recipientInput ||
               u.phone.replace(/\D/g,'') === recipientInput.replace(/\D/g,'')
        );
        if (found) {
          setRecipientPhone(found.phone);
          setRecipientName(found.fullName);
        } else {
          setRecipientPhone(recipientInput);
      ...
      // Purpose is optional — proceed regardless
      ...
      await speak(
        `You are about to send ${formatNaira(amtNum)} to ${recipientName}.` +
        (transferType === 'protected' ? ` ${L.send_protected_info}` : '')
      );
      setStep('confirm');
  ...
  const STEP_NUMS: Record<Step, number> = {
    recipient: 0, amount: 1, purpose: 2, type: 3, pin: 4, confirm: 5, done: 6,
  };
    ...
    <div className="phone-frame bg-surface-light">
      <div className="flex-shrink-0" style={{ background: 'linear-gradient(135deg, #0D1B3E, #FF6B35)' }}>
        <div className="flex items-center gap-3 px-4 pt-12 pb-5">
          <button
            onClick={() => step === 'recipient' ? navigate('dashboard') : setStep(prevStep())}
            className="text-white/80 text-sm p-2 -ml-2"
          >← {L.back}</button>
          <h1 className="text-white font-bold text-lg flex-1">Send Money</h1>
          {step !== 'pin' && step !== 'confirm' && step !== 'done' && (
            <MicButton onClick={tapMic} size="sm" />
          )}
        </div>
        {/* Progress */}
        <div className="flex gap-1 mx-4 mb-4">
          {(['recipient','amount','purpose','type','pin','confirm'] as Step[]).map((s, i) => (
            <div key={s} className="flex-1 h-1 rounded-full transition-all"
                 style={{ background: STEP_NUMS[step] >= i ? '#00C27C' : 'rgba(255,255,255,0.2)' }} />
          ))}
        </div>
      </div>
        ...
        <div className="mx-6 mt-4 animate-slide-up" key={step}>
          {step === 'recipient' && (
            <>
              ...
              {recipientPhone && (
                <div className="flex items-center gap-2 mt-2 px-3 py-2 rounded-xl" style={{ background: '#D6F5EA' }}>
          ...
          {step === 'purpose' && (
            <>
              <p className="text-sm font-semibold text-ink-secondary mb-3">What is this payment for? <span className="text-ink-muted font-normal">(optional)</span></p>
              <input
                autoFocus
                value={purpose}
                onChange={e => setPurpose(e.target.value)}
                placeholder="e.g. Rent, School fees, Business payment…"
                className="w-full px-4 py-4 rounded-2xl text-base border-2 transition-all"
                style={{ borderColor: purpose ? '#00C27C' : '#CBD3E8', background: '#fff', color: '#0D1B3E' }}
              />
              <p className="text-xs text-ink-muted mt-2">This will be visible in both accounts' transaction history.</p>
            </>
          )}
          ...
          {step === 'type' && (
            <>
              ...
              {([
                { type: 'standard' as const, icon: '⚡', title: 'Send Immediately', desc: 'Funds arrive instantly. Cannot be recalled.' },
                { type: 'protected' as const, icon: '🛡', title: 'Protected Payment', desc: 'Funds held securely until you confirm delivery.' },
              ]).map(opt => (
          ...
          {step === 'confirm' && (
            <div className="bg-white rounded-2xl p-5 card">
              ...
              {[
                ...
                { label: 'Type',    value: transferType === 'protected' ? '🛡 Protected' : '⚡ Immediate' },
          ...
          {step === 'done' && (
            <div className="text-center pt-4">
              ...
              <p className="text-sm text-ink-muted mb-1">
                {formatNaira(amountNum)} {transferType === 'protected' ? 'held securely for' : 'sent to'}
              </p>
              <p className="font-bold text-ink-primary mb-1">{recipientName}</p>
              {purpose && <p className="text-xs text-ink-muted">Purpose: {purpose}</p>}
              {transferType === 'protected' && (
                <p className="text-xs text-ink-muted mt-2 px-6">Funds will be released when you confirm delivery.</p>
              )}
            </div>
          )}
